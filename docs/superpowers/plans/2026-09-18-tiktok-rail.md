...
- [x] Tests first: advertisers mapping + sort + 10-min cache (second call = no fetch) + empty answer never cached; config mapping; reads retry once on 5xx, launches do NOT (exactly one request on a 500); 4xx body → `TiktokWeaponError` with the partner's sentence and status; bearer header present; trailing slashes on every path; **live guard**: base = production host and no `TIKTOK_ALLOW_LIVE_LAUNCH`/`VERCEL_ENV` → the launch throws `tiktok_live_launch_blocked` with ZERO fetch calls, while reads still go out; mock base → allowed; `twTasks` isolates a per-id 404 as `not_found`.
- [x] Run → FAIL; implement; run → PASS. Commit.
...
- [x] Every route: session → `tiktokRailEnabled()` 404 → `tiktokWeaponConfigured()` 500. Launch routes `maxDuration = 800`.
...
- [x] `tsc --noEmit` clean, `eslint` clean on new files. Commit.
...
- [x] Card helpers tested first: `freshTiktokCard`, `cloneTiktokCard`, `buildTiktokShot` (dry-run placeholders vs real uploads, copies fan-out), `tiktokCardRefusal`, `tiktokCardSignature`.
- [x] Board: advertisers + per-card config hooks, Blob uploads 3 at a time (videos + avatar), one POST, unload guard only while uploading.
...
- [x] `node --test tests/*.test.ts` (whole suite — other rails untouched), `npx tsc --noEmit`, `npx eslint` on changed files, `npx next build`.
- [x] Mock + smoke green twice in a row.
- [x] Browser run of `/tiktok` and `/tiktok/clone` through `_e2e/_adl_session_proxy.mts` against the mock (screens in `_e2e/_ui_shots`).
- [x] Flag OFF build check: tab disabled, `/tiktok` → `/`, `/api/tiktok/*` → 404.
- [x] Code review (code-reviewer agent) → fix → re-verify. Update the spec status line.
