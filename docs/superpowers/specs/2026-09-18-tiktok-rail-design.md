...
**Date:** 2026-09-18 · **Status:** design approved by the owner (scope, accounts, row outcome, assets) · **Branch:** `feat/tiktok-rail` — local only, never pushed (a push to `main` auto-deploys on Vercel).
...
- **Settle:** after the last submit, sent tasks are polled every 10 s (≤5 in flight) for up to 6 min or until the deadline margin; `completed` → `done/created` + `campaign_id` + real name; `failed` → `error/lion` + `error_step: error_message`; unknown reads never downgrade a row.

### 4.3 UI

`/tiktok` — cards (add / duplicate / remove / collapse, copies 1–10 per card) + sticky Launch bay (readiness rows, total/day, Preview → Launch), the FB/Google/Snap launcher shape. Card rows: Advertiser · Pixel · Mode · Budget → Countries (advertiser config; presets World/LATAM/Anglo/Franco filtered to what the account allows) · Language · Bid/ROAS → Landing (datalist of the team's live landings + final-link preview + Copy) → Campaign name (read-only LION head + custom tail) · MOSH → Identity (name + avatar drop, auto-crop 256² PNG, last 8 identities remembered in localStorage with their hosted URLs) → Creative (ad text ≤100 with counter, CTA, 1–20 videos in the portrait 9:16 strip, or pasted https URLs) → Smart+ panel (budget level, extra texts/CTAs). The readiness dot is `tiktokLaunchWire` itself run on placeholder URLs, so the card can't disagree with the server. Files upload to Blob 3 at a time at launch; the unload guard is mounted only then.
...
Task manager — the Google drawer's twin: shared team view, owner authority, stage labels `Sending → Sent to LION → Created · cmp <id>` / `Failed at <step>`, copy campaign id, client finisher for the owner's own sent rows.
