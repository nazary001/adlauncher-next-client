...
import { TiktokCloneBoard } from "@/components/tiktok-clone-board";

export const metadata: Metadata = {
  title: "TikTok — clone & JURO — Ad Launcher",
};

/** Dedupe + trim the comma-separated TikTok campaign ids handed over in the link (digits only). */
function parseIds(raw: string | string[] | undefined): string[] {
  if (!raw) return [];
  const joined = Array.isArray(raw) ? raw.join(",") : raw;
  return [...new Set(joined.split(",").map((x) => x.trim()).filter((x) => TIKTOK_CAMPAIGN_ID_RE.test(x)))];
}

export default async function TiktokClonePage({
  searchParams,
}: {
  ...
  // to /tiktok/clone must not open a half-wired board — bounce to the launcher.
