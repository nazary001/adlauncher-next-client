...
import { isSnapKey } from "@/lib/snap-launch";
import { SNAP_KEY_POOL_SIZE, listSnapKeys, releaseSnapKeyByKey, snapFreeKeys, snapNextKey } from "@/lib/snap-keys";
 ...
 *  without Snapchat credentials (the registry is ours) — only the rail flag gates it. `poolMax`
 *  comes from the registry module itself, so it always matches the `free` list it computes. */
export async function GET(req: Request): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, poolMax: SNAP_KEY_POOL_SIZE, used, free, next: snapNextKey(used.map((r) => r.key)) });
  } catch (e) {
 ...
 *  `released: false` means no row existed; a Strapi failure is a 502, never a false "released". */
