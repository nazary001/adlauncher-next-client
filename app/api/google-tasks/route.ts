...
/** GET → the team's Google tasks from the last 7 days (shared view). */
...
/** POST → upsert one Google task by task_id (partner forced to "gg", owner from the session). */
...
/** DELETE ?taskIds=… → remove the caller's Google rows (foreign rows skipped). Kept for
